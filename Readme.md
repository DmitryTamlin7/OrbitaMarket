...
В репозиторий интегрирован базовый CI-пайплайн автоматического тестирования изменений (`.github/workflows/ci.yml`).

При каждом `git push` или создании `Pull Request` в ветки `main` GitHub запускает виртуальную машину на Ubuntu, разворачивает окружение **JDK 21** и выполняет полную компиляцию кодовой базы вместе с прогоном модульных и интеграционных тестов (`./gradlew build`) отдельно по каждому сервису
1. **API Gateway**
2. **Payments Service**
3. **Order Service**
